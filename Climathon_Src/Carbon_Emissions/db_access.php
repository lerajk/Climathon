...
function retrieve_carbon_level($latitude,$longitude)
{
 ...
 //SELECT * FROM table WHERE primary_key IN (value1, value2
  $query = "SELECT * FROM satellite_data_5 WHERE Lat_ID=$sql_lat AND Lng_ID=$sql_lng";
  $result = mysql_query($query) or die('Query failed: ' . mysql_error());
  
  $num_results = mysql_num_rows($result);

  $closest_distance = (double)999999;
  $carbon_level = -1;
  ...
  for($i = 0; $i < $num_results; $i++)
  {
   ...
   $temp_distance = (double)sqrt(((double)$latitude - (double)$temp_lat)^2 + ((double)$longitude - (double)$temp_lng)^2);

   if($temp_distance <= $closest_distance) 
   {
     $closest_distance = $temp_distance;
     $carbon_level = $row['CO2'];
   }
